...
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `AddBooking`(in customer_id int , in table_no int, in booking_date date)
begin
	insert into bookings(customerid, tableno, bookingdate)
    values (customer_id, table_no, booking_date);
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `AddValidBooking`(in customer_id int, in book_date date, in table_no int)
begin
    ...
    where bookingdate = book_date and tableno = table_no;
    ...
    if check_booking > 0 then 
        ...
        values (customer_id, book_date, table_no);
        commit;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `CancelBooking`(in booking_id int)
begin
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `UpdateBooking`(in booking_id int, in booking_date date)
begin
	update bookings
...
-- Dump completed on 2025-05-08 15:42:44
